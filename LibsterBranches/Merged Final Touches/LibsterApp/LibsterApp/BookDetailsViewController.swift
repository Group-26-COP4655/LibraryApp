        ...
        queryThree.getObjectInBackground(withId: tempObjIDThree) { (userFields: PFObject?, error: Error?) in
             if let error = error {
                print(error.localizedDescription)
                 ...
                 self.setRating(userRating!, true)
            }
        }
    }
...
//                    print("Book has successfully been removed from favorites!")
    ...
    func setRating(_ toRate:Int, _ inViewDidLoad:Bool){
            ...
            do {
                ...
                query.getObjectInBackground(withId: tempObjID) { (userFields: PFObject?, error: Error?) in
                     if let error = error {
                        ...
                        var newCumulativeRating:Double = 0.0
                        var newRaters:Double = 0.0
                        var newRatingUnrounded:Double = 0.0
                        var newRatingDecimalPointShiftRight:Double = 0.0
                        var newRatingAsWholeNumber:Double = 0.0
                        var newRatingRounded:Double = 0.0

                        newCumulativeRating = Double(cumulativeRating + toRate)
                        newRaters = Double(raters + 1)
                        newRatingUnrounded = newCumulativeRating / newRaters
                        newRatingDecimalPointShiftRight = newRatingUnrounded * 10
                        newRatingAsWholeNumber = round(newRatingDecimalPointShiftRight)
                        newRatingRounded = newRatingAsWholeNumber / 10
                        ...
                        userFields["rating"] = String(newRatingRounded)
                        userFields.saveInBackground()

                        let ratingText = String(newRatingRounded) + "/5"
